...
public class ColorSlideTransitionHelper
{
    ...
    public ColorSlideTransitionHelper(UIElement hostForVisual)
    {
        ...
        this._hostForVisual = hostForVisual;
    ...
    {
        SpriteVisual colorVisual = CreateVisualWithColorAndPosition(color, finalBounds);
        ...
        Rect finalBounds)
    {
        ...
        Vector3 offset = new((float)finalBounds.Left, (float)finalBounds.Top, 0f);
        Vector2 size = new((float)finalBounds.Width, (float)finalBounds.Height);
    ...
    private void TriggerSlideAnimation(SpriteVisual colorVisual)
    {
        ...
        CompositionScopedBatch batchTransaction = _compositor.CreateScopedBatch(CompositionBatchTypes.Animation);
    ...
    private void SlideAnimationCompleted(object sender, CompositionBatchCompletedEventArgs args)
    {
        if (!_firstRun)
        {
            foreach (Visual childVisual in _containerForVisuals.Children)
            {
