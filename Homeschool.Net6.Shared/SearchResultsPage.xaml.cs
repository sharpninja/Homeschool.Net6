...
public sealed partial class SearchResultsPage : ItemsPageBase
{
    ...
    protected override void OnNavigatedTo(NavigationEventArgs e)
    {
        ...
        string queryText = e.Parameter != null
            ? e.Parameter.ToString().ToLower()
            : null;
    ...
    private void BuildFilterList(string queryText)
    {
        if (!string.IsNullOrEmpty(queryText))
        {
            ...
            foreach (ControlInfoDataGroup group in ControlInfoDataSource.Instance.Groups)
            {
                ...
                {
                    filterList.Add(new(group.Title, numberOfMatchingItems, matchingItems));
                }
            }
            ...
            else
            {
                ...
                filterList.Insert(0, new("All", allControls.Count, allControls, true));
                Filters = filterList;
...
public sealed class Filter : INotifyPropertyChanged
{
    ...
    public string Name
    {
        ...
        set { if (SetProperty(ref _name, value))
            {
                NotifyPropertyChanged(nameof(Filter.Description));
            }
        }
    }
    ...
    {
        get => _count;
        set { if (SetProperty(ref _count, value))
            {
                NotifyPropertyChanged(nameof(Filter.Description));
            }
        }
